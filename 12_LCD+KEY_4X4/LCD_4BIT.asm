;##################################################################################################
;	SUBRUTINAS en este PAQUETE:
;
;	LCD_Inicializa					Se debe emplear antes que cualquiera otra para asignar recursos al LCD.
;	LCD_EscribeLCD					Envía el contenido de "W" hacia el LCD.
;	LCD_EnviaComando				Envía un Comando al LCD, por medio de "W".
;	LCD_Caracter						Escribe en Memora DDRAM el caracter ASCII depositado en "W".
;	LCD_Envia								Escritura genérica hacia el LCD.
;	LCD_CodigoCGROM					Convierte códigos ASCII para visualizarse en el LCD correctamente.
;	LCD_LineaEnBlanco				Borra (pone espacios en blanco) en la línea actualmente seleccionada.
;	LCD_UnEspacioBlanco			Imprime un espacio en blanco a partir de la posición del cursor.
;	LCD_DosEspaciosBlancos	Imprime dos espacios en blanco a partir de la posición del cursor.
;	LCD_TresEspaciosBlancos Imprime tres espacios en blanco a partir de la posición del cursor.
;	LCD_EnviaBlancos				Imprime espacios en blanco, en donde "W" indica el número de espacios.
;	LCD_Nibble							Imprime en la actual posición del cursor el valor Hexadecimal de 
;													la parte baja (4 bits menos significativos) del registro "W".
;	LCD_ByteCompleto				Imprime el contenido del registro "W" como dos caracteres 
;													Hexadecimales, uno por cada "nibble" (Parte Alta + Parte Baja).
;	LCD_Byte								Igual que "LCD_ByteCompleto", sólo que si el "nibble" Alto estuviera
;													en cero (b'0000'), en su lugar imprimirá un espacio en blanco (' ')
;
; COMANDOS para el LCD:
;	LCD_CursorIncr				Autoincrementa el cursor (b'00000110')
;	LCD_Linea1						Cursor al principio de la Línea 1 (b'10000000')
;	LCD_Linea2						Cursor al principio de la Línea 2 (b'11000000')
;	LCD_Linea3						Cursor al principio de la Línea 3 (b'10010100')
;	LCD_Linea4						Cursor al principio de la Línea 4 (b'11010100')
;	LCD_PosicionLinea1		Cursor a (Línea 1 + Valor de "W")
;	LCD_PosicionLinea2		Cursor a (Línea 2 + Valor de "W")
;	LCD_OFF								Pantalla Apagada
;	LCD_CursorON					Pantalla y Cursor encendidos
;	LCD_CursorOFF					Pantalla encendida, Cursor apagado
;	LCD_Borra							Pantalla y Memoria DDRAM borradas, Cursor a principio de Línea 1
;	LCD_2Lineas4Bits5x7		Definición de LCD de 2 Líneas, comunicación de 4 bits, matriz 5X7
;
...
; REVISAR este valor para cristales de 20 MHz
	call	R_20mS							; Este retardo es necesario para simular en PROTEUS.
...
;
; Envía el dato del registro de trabajo W al bus de datos y produce un pequeño pulso en el pin
...
LCD_Borra:									; Borra toda la pantalla, memoria DDRAM y pone el 
	...
	goto	LCD_EnviaComando
LCD_2Lineas4Bits5x7:				; Define la pantalla de 2 líneas, con caracteres de 5X7
	movlw	b'00101000'					; puntos y conexión al PIC mediante bus de 4 bits. 
...
LCD_Envia:
	...
	call	R_2mS								; 2 ms en modo comando y 50 µs en modo cáracter.
	call	R_50uS
